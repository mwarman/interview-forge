...
describe('useCreatePlan', () => {
  ...
  it('should successfully trigger plan generation', async () => {
    ...
    // Act
    const { result } = renderHookWithAllProviders(() => useCreatePlan(JD_ID, SESSION_ID));
    ...
    expect(apiClient.post).toHaveBeenCalledWith(`/jds/${JD_ID}/sessions/${SESSION_ID}/plan`);
  });

  it('should handle API errors', async () => {
    // Arrange
    const apiError = new ApiError('Failed to create plan', 500);
