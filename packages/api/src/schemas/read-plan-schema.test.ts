...
import { readPlanActionEventSchema, type ReadPlanActionEvent } from './read-plan-schema';

describe('readPlanActionEventSchema', () => {
  describe('valid events', () => {
    it('should accept valid ReadPlanAction event with jdId and sessionId', () => {
      // Arrange
      const event = {
      ...
      // Act
      const result = readPlanActionEventSchema.safeParse(event);

      // Assert
      expect(result.success).toBe(true);
      if (result.success) {
        expect(result.data.actionGroup).toBe('interview-forge-read-plan');
        expect(result.data.function).toBe('read-plan-action');
    ...
    it('should infer correct type for valid event', () => {
      // Arrange
      const event: ReadPlanActionEvent = {
        actionGroup: 'interview-forge-read-plan',
        function: 'read-plan-action',
        parameters: [
          {
